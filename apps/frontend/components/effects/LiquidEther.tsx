  ...
  useEffect(() => {
    ...
    function makePaletteTexture(stops: string[]): THREE.DataTexture {
      const safeStops = Array.isArray(stops)
        ? stops.filter((c): c is string => typeof c === "string")
        : [];

      const arr =
        safeStops.length === 1 ? [safeStops[0], safeStops[0]] : safeStops;
    ...
    const paletteTex: THREE.DataTexture = makePaletteTexture(colors);
...
`;
    ...
    class Advection extends ShaderPass {
      ...
      update = (...args: any[]) => {
        ...
        if (!uniforms || !this.line) return;

        if (typeof dt === "number" && uniforms.dt) {
        ...
        if (typeof BFECC === "boolean" && uniforms.isBFECC) {
          uniforms.isBFECC.value = BFECC;
        }
        super.update();
      };
    }
    ...
    class Viscous extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        if (typeof viscous === "number" && uniforms.v) {
          uniforms.v.value = viscous;
        }
